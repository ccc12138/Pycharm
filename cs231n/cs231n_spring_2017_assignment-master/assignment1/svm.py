...
num_learning = len(range_lr)
num_regularization = len(range_reg)
for i in range(num_learning):
    for j in range(num_regularization):
        svm = LinearSVM()
        loss_hist = svm.train(X_train, y_train, learning_rate=range_lr[i], reg=range_reg[j],
        ...
        results[(range_lr[i], range_reg[j])] = (accuracy_train, accuracy_val)
